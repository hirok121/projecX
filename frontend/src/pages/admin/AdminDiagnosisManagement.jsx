...
function AdminDiagnosisManagement() {
  ...
  const fetchDiagnosisData = async () => {
    setLoading(true);
    try {
      // Fetch all diagnoses using admin endpoint
      const response = await diagnosisAPI.getAllDiagnosesAdmin();
      setDiagnoses(response || []);
      setError(null);
    } catch (err) {
      logger.error("Diagnosis data fetch error:", err);
      setError(err);
    } finally {
      setLoading(false);
    }
    ...
    <Box sx={{ minHeight: "100vh", bgcolor: "grey.50" }}>
      ...
      <Container maxWidth="xl" sx={{ py: 4 }}>
        ...
        <Box mb={4}>
          ...
          >
            View and manage all diagnosis records across the system
          </Typography>
          <Typography
            variant="body2"
            color="text.secondary"
          ...
          >
            Monitor all user diagnoses, track system-wide statistics, and access detailed 
            information about each diagnosis including disease insights, classifier details, 
            and prediction results. Click on any diagnosis to view complete details.
          </Typography>
        </Box>
        ...
        {diagnoses.length === 0 && (
          <Card sx={{ p: 4, textAlign: "center" }}>
            <Alert severity="info">
              No diagnosis records found in the system.
            </Alert>
          </Card>
        )}
        ...
        <Box mt={4} mb={4}>
          <Card sx={{ p: 3, bgcolor: "warning.50", borderLeft: 4, borderColor: "warning.main" }}>
            ...
            <Typography variant="body2" color="text.secondary">
              This admin panel provides access to all diagnosis records in the system. 
              Please handle all patient data with care and in compliance with privacy 
              regulations. All diagnoses are generated by AI models for research and 
