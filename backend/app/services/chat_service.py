...
class ChatService:
    ...
    @staticmethod
    def create_chat(db: Session, user_id: int, title: str, description: str = None) -> Chat:
    ...
    def get_chat_with_messages(
        ...
        try:
            ...
            app_logger.info(f"Retrieved chat {chat_id} with {len(chat.messages)} messages")
            return chat
    ...
    @staticmethod
    def update_chat_title(db: Session, chat_id: int, user_id: int, new_title: str) -> Optional[Chat]:
        """Update chat title."""
        try:
            chat = db.query(Chat).filter(
                and_(Chat.id == chat_id, Chat.user_id == user_id)
    ...
    @staticmethod
    def archive_chat(db: Session, chat_id: int, user_id: int, archived: bool = True) -> bool:
    ...
    def get_chat_stats(db: Session, user_id: int) -> dict:
        ...
        try:
            ...
            archived_chats = db.query(Chat).filter(
                and_(Chat.user_id == user_id, Chat.is_archived == True)
            ).count()
            pinned_chats = db.query(Chat).filter(
                and_(Chat.user_id == user_id, Chat.is_pinned == True)
            ).count()
